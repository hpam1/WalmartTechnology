...
public class SeatDAOUtil {
	static Logger log = LogManager.getLogger(SeatDAOUtil.class);
	
	/*
	 * count the number of available seats in the venue
	 * 
	 *  @param venue - the performance venue
	 *  @return the count of available seats in the venue
	 */
	public static int countAvailSeatInVenue(Venue venue) {
		log.debug("SeatDAOUtil countAvailSeatInVenue " + venue);
		if(venue == null)
			return 0;
	 ...
	 * 
	 * @param venue - the performance venue
	 * @param levelId - the venue level id
	 * @return the count of available seats in the venue
	 * 
	 * If a level Id that does not exist in the venue is given
	 * as input, this method will return 0
	 * 
	 */
	public static int countAvailSeatInLevel(Venue venue, int levelId) {
		log.debug("SeatDAOUtil countAviailSeatInLevel " + venue + " " + levelId);
		if(venue == null)
			return 0;
	 ...
	 * 
	 * @param list of venue levels whose available seats are to be computed
	 * 
	 */
	...
	/*
	 * count the seats available, optionally limited by levels
	 * If minLevel is present, then search for available seats from this level
	 * If maxLevel is present, then search for available seats within this level
	 * otherwise, search the entire venue
	 *  
	 */
	public static int countAvailSeatsWithinLevels(Venue venue, Optional<Integer> minLevel, Optional<Integer> maxLevel) {
		log.debug("SeatDAOUtil countAvailSeatsWithinLevels");
		if(venue == null)
			return 0;
		List<Level> levelList = venue.getVenueLevels();
		if(levelList == null || levelList.size() == 0)
	...
	/*
	 * get the available seat list, optionally limited by levels
	 * If minLevel is present, then search for available seats from this level
	 * If maxLevel is present, then search for available seats within this level
	 * otherwise, search the entire venue 
	 */
	private static List<Seat> getAvailSeatsInLevels(Venue venue, Optional<Integer> minLevel, Optional<Integer> maxLevel) {
		...
		List<Seat> seatList = levelList.stream().flatMap(lvl -> lvl.getSeatList().stream()).collect(Collectors.toList());
		if(seatList != null && seatList.size() > 0) {
			seatList = seatList.stream().filter(seat -> seat.getStatus().equals(SeatStatus.AVAILABLE)).collect(Collectors.toList());
		}
		return seatList;
	}
	
	/*
	 * randomly choose an available seat in the venue, optionally limited by levels, and
	 * hold it for a user.
	 * 
	 */
	public static synchronized Seat holdARandomSeat(Venue venue, Optional<Integer> minLevel, Optional<Integer> maxLevel) {
		if(venue == null)
			return null;
