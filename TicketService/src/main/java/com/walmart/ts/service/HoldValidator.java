 ...
 * 
 * HoldValidator clears SeatHolds that are older than a threshold value and 
 * releases the held seats to other users
 * 
 * SeatHoldList is a linkedlist whose elements are sorted by the hold creation time
 * with the element at the head of the list representing the oldest hold and that at
 * the tail represents the newest hold
 * Iterate through the seatholdlist from the head till the element whose hold creation
 * time is within the threshold (in seconds)
 * 
 *
 */
public class HoldValidator {
	static Logger log = LogManager.getLogger(HoldValidator.class);
	// default threshold time is 60 seconds
	static long holdThreshold = 60l;
	/*
	 * read the hold expiration threshold value from a properties file TSProperties.properties
	...
	static {
		...
		try {
			...
			log.error("Error reading TSProperties file for hold threshold " + e.getMessage());
		}
		log.info("Hold threshold in seconds: " + holdThreshold);
	}
	
	/*
	 * clear the holds that are more than threshold seconds from the current time
	 */
	public static void purgeExpiredHolds() {
		LinkedList<SeatHold> seatHoldList = SeatHoldRepository.getSeatHoldList();
		log.info("Start: In HoldValidator purging expired holds; no of holds: " + seatHoldList.size());
		...
		while(seatHoldList != null && seatHoldList.size() > 0) {
		    ...
		    } else { // the current element within the threshold and so are the next elements; break the processing
		    	break;
		    }
		}
		log.info("End: In HoldValidator; no of holds: " + seatHoldList.size());
	}
}
